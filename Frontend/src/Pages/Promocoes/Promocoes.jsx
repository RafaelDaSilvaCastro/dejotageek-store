  ...
  const postPromocoes = async () => {
    try {
      ...
      const response = await blogFetch.post(
        `/promotions/create?productId=${proudctId}`,
        formData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
    ...
    <div className="flex">
      ...
      <div className="flex-1 p-4 rounded-md mr-20">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="bg-vermelho-botao text-white drop-shadow-[0px_3px_7px_rgba(0,0,0,0.25)] rounded-lg px-4 py-3 flex items-center hover:scale-105 duration-150 mt-4"
          >
            Adicionar Promoção
          </button>
        </form>
        ...
        <ul className="list-disc list-inside">
          {promocoes.map((promocao, index) => (
            <li key={index} className="mb-2">
              <strong>{promocao.description}</strong> de {promocao.percentage}%
              de desconto para o produto {promocao.productName}
              <br />
              Válido de {transformDate(promocao.startDate)} até{" "}
              {transformDate(promocao.endDate)}
            </li>
          ))}
        </ul>
      </div>
